...
in “Cargo Workspaces” on page 307.
...
page 5), the compiler considers that file to be a crate. Crates can contain
...
Modules to Control Scope and Privacy” on page 123).
...
submodules. For example, you might declare `mod vegetables;` in
...
privacy rules allow, using the path to the code. For example, an `Asparagus`
...
default. To make a module public, declare it with `pub mod` instead of `mod`.
...
page 127, we’ll go into more detail about `pub`.
...
crate::front`_of_house::hosting` and then called `hosting::add_to_waitlist` in
...
`restaurant::front_of_house::hosting::add_to`_waitlist()`. Now that this `pub
...
Convenient Public API with pub use” on page 300.
...
22, we brought the `Rng` trait into scope and called the `rand::thread_rng`
function:
...
216. The glob operator is also sometimes used as part of the prelude pattern:
...
Item in the Module Tree” on page 125. In other words, `mod` is *not* an
