...
In “Dangling References” on page 75, we mentioned that the compiler ensures
...
Recall from “The Slice Type” on page 77 that a slice is a pointer to some data
...
page 33. The names of static variables are in `SCREAMING_SNAKE_CASE` by
...
“Extensible Concurrency with the Send and Sync Traits” on page 373: the
...
We first covered traits in “Traits: Defining Shared Behavior” on page 192, but
...
Pattern to Implement External Traits” on page 439. We want to add values in
millimeters to values in meters and have the implementation of `Add` do the
conversion correctly. We can implement `Add` for `Millimeters` with `Meters` as
...
1. 1.To extend a type without breaking existing code
1. 2.To allow customization in specific cases most users won’t need
...
`outline`_print` method that will print a given value formatted so that it’s
...
In “Implementing a Trait on a Type” on page 193, we mentioned the orphan rule
...
Without Named Fields to Create Different Types” on page 89.) The tuple struct
...
Pointers Like Regular References with Deref” on page 321). If we didn’t want
...
to Implement External Traits” on page 439. The newtype pattern is also useful
for tasks beyond those we’ve discussed so far, including statically enforcing
that values are never confused and indicating the units of a value. You saw an
example of using newtypes to indicate units in Listing 19-15: recall that the
`Millimeters` and `Meters` structs wrapped `u32` values in a newtype. If we
wrote a function with a parameter of type `Millimeters`, we wouldn’t be able to
...
Implementation Details” on page 376.
...
Flow Construct” on page 110, we discussed that `match` arms must all return the
...
page 79 that the slice data structure just stores the starting position and the
...
Different Types” on page 379, we mentioned that to use traits as trait objects,
...
“Advanced Traits” on page 430 because there are multiple functions available
named `to_string`.
...
Recall from “Enum Values” on page 104 that the name of each enum variant that
we define also becomes an initializer function. We can use these initializer
...
`hello_macro`_derive` to *https://crates.io*, they would be regular
...
with macro_rules! for General Metaprogramming” on page 449. Function-like
macros take a `TokenStream` parameter, and their definition manipulates that
